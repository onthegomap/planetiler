...
/**
 * Performance tests for {@link LongLongMap} implementations. Adds items to the map then reads from
 * it for 30s using multiple threads and reports memory/disk usage, writes and reads per second.
 */
  ...
  public static void main(String[] args) throws InterruptedException {
    ...
    new Worker(
            ...
            () -> {
              ...
              String rate =
                  format.numeric(entries * NANOSECONDS_PER_SECOND / (end - start), false) + "/s";
              System.err.println(
                  "Loaded "
                      + entries
                      + " in "
                      + Duration.ofNanos(end - start).toSeconds()
                      + "s ("
                      + rate
                      + ")");
              writeRate.set(rate);
    ...
    loggers =
        ProgressLoggers.create().addRateCounter("entries", readCount).newLine().addProcessStats();
