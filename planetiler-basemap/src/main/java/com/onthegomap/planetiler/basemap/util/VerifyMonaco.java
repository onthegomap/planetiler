...
public class VerifyMonaco {
  ...
  /**
   * Returns a verification result with a basic set of checks against an openmaptiles map built from
   * an extract for Monaco.
   */
  public static Verify verify(Mbtiles mbtiles) {
    ...
    verify.checkMinFeatureCount(
        MONACO_BOUNDS, "transportation", Map.of(), 10, 14, 5, LineString.class);
    verify.checkMinFeatureCount(
        MONACO_BOUNDS,
        "landcover",
        Map.of(
        ...
        Polygon.class);
    verify.checkMinFeatureCount(
        MONACO_BOUNDS, "water", Map.of("class", "ocean"), 0, 14, 1, Polygon.class);
    verify.checkMinFeatureCount(
        MONACO_BOUNDS, "place", Map.of("class", "country"), 2, 14, 1, Point.class);
    return verify;
  }
