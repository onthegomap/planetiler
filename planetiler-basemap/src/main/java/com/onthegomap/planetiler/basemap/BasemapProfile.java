...
/**
 * Delegates the logic for generating a map to individual implementations in the {@code layers}
 * package.
 *
 * <p>Layer implementations extend these interfaces to subscribe to elements from different sources:
 *
 * <ul>
 *   <li>{@link LakeCenterlineProcessor}
 *   <li>{@link NaturalEarthProcessor}
 *   <li>{@link OsmWaterPolygonProcessor}
 *   <li>{@link OsmAllProcessor} to process every OSM feature
 *   <li>{@link OsmRelationPreprocessor} to process every OSM relation during first pass through OSM
 *       file
 *   <li>A {@link Tables.RowHandler} implementation in {@code Tables.java} to process input features
 *       filtered and parsed according to the imposm3 mappings defined in the OpenMapTiles schema.
 *       Each element corresponds to a row in the table that imposm3 would have generated, with
 *       generated methods for accessing the data that would have been in each column
 * </ul>
 *
 * Layers can also subscribe to notifications when we finished processing an input source by
 * implementing {@link FinishHandler} or post-process features in that layer before rendering the
 * output tile by implementing {@link FeaturePostProcessor}.
 */
public class BasemapProfile extends ForwardingProfile {

  // IDs used in stats and logs for each input source, as well as argument/config file overrides to
  // source locations
  ...
  /**
   * Index to efficiently find the imposm3 "table row" constructor from an OSM element based on its
   * tags.
   */
  private final MultiExpression.Index<RowDispatch> osmMappings;
  /**
   * Index variant that filters out any table only used by layers that implement IgnoreWikidata
   * class.
   */
  ...
  public BasemapProfile(Translations translations, PlanetilerConfig config, Stats stats) {
    List<String> onlyLayers =
        config.arguments().getList("only_layers", "Include only certain layers", List.of());
    List<String> excludeLayers =
        config.arguments().getList("exclude_layers", "Exclude certain layers", List.of());
    ...
    for (Layer layer : OpenMapTilesSchema.createInstances(translations, config, stats)) {
      if ((onlyLayers.isEmpty() || onlyLayers.contains(layer.name()))
          && !excludeLayers.contains(layer.name())) {
        layers.add(layer);
    ...
    for (Handler handler : layers) {
      if (handler instanceof NaturalEarthProcessor processor) {
        registerSourceHandler(
            ...
            (source, features) ->
                processor.processNaturalEarth(source.getSourceLayer(), source, features));
      }
    ...
    // pre-process layers to build efficient indexes for matching OSM elements based on matching
    // expressions
                      ...
                      handlerMap.getOrDefault(constructor.rowClass(), List.of()).stream()
                          .map(
                              r -> {
                                ...
                                var handler = (Tables.RowHandler<Tables.Row>) r.handler();
                                return handler;
                              })
                          .toList();
            ...
            .mapResults(
                constructor ->
                    handlerMap.getOrDefault(constructor.rowClass(), List.of()).stream()
                        .anyMatch(
                            handler ->
                                !IgnoreWikidata.class.isAssignableFrom(handler.handlerClass())))
            .filterResults(b -> b)
            .simplify()
            .index();
  ...
  /**
   * Layers should implement this interface to subscribe to elements from <a
   * href="https://www.naturalearthdata.com/">natural earth</a>.
   */
  public interface NaturalEarthProcessor {

    /**
     * Process an element from {@code table} in the<a
     * href="https://www.naturalearthdata.com/">natural earth source</a>.
     *
  ...
  /**
   * Layers should implement this interface to subscribe to elements from <a
   * href="https://github.com/lukasmartinelli/osm-lakelines">OSM lake centerlines source</a>.
   */
  public interface LakeCenterlineProcessor {

    /**
     * Process an element from the <a href="https://github.com/lukasmartinelli/osm-lakelines">OSM
  ...
  /**
   * Layers should implement this interface to subscribe to elements from <a
   * href="https://osmdata.openstreetmap.de/data/water-polygons.html">OSM water polygons source</a>.
   */
  public interface OsmWaterPolygonProcessor {

    /**
     * Process an element from the <a
     * href="https://osmdata.openstreetmap.de/data/water-polygons.html">OSM water polygons
     * source</a>
     *
     * @see Profile#processFeature(SourceFeature, FeatureCollector)
     */
  ...
  /**
   * Layers should implement to indicate they do not need wikidata name translations to avoid
   * downloading more translations than are needed.
   */
  public interface IgnoreWikidata {}

  private record RowDispatch(
      Tables.Constructor constructor, List<Tables.RowHandler<Tables.Row>> handlers) {}
}
