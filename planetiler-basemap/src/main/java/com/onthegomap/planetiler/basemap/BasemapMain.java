...
public class BasemapMain {
  ...
  static void run(Arguments arguments) throws Exception {
    ...
    // use --area=... argument, AREA=... env var or area=... in config to set the region of the
    // world to use
    // will be ignored if osm_path or osm_url are set
    String area =
        arguments.getString(
            "area",
            "name of the extract to download if osm_url/osm_path not specified (i.e. 'monaco'"
                + " 'rhode island' 'australia' or 'planet')",
            "monaco");
    ...
    Planetiler.create(arguments)
        ...
        .addShapefileSource(
            "EPSG:3857",
            BasemapProfile.LAKE_CENTERLINE_SOURCE,
            sourcesDir.resolve("lake_centerline.shp.zip"),
            // was previously using this old build from 2016:
