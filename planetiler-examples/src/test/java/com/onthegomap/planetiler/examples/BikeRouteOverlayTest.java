...
public class BikeRouteOverlayTest {
  ...
  public void testSourceFeatureProcessing() {
    ...
    var relationResult =
        profile.preprocessOsmRelation(
            new OsmElement.Relation(
                1,
                Map.of(
                    "type", "route",
                    "route", "bicycle",
                    ...
                    "network", "lcn",
                    "ref", "1"),
                List.of(new OsmElement.Relation.Member(OsmElement.Type.WAY, 2, "role"))));
    ...
    var way =
        SimpleFeature.createFakeOsmFeature(
            ...
            null,
            2,
            relationResult.stream()
                .map(info -> new OsmReader.RelationMember<>("role", info))
                .toList());
  ...
  public void testTilePostProcessingMergesConnectedLines() {
    ...
    assertEquals(
        List.of(connected), profile.postProcessLayerFeatures(layer, 14, List.of(line1, line2)));
  }

  @Test
  public void integrationTest(@TempDir Path tmpDir) throws Exception {
    Path dbPath = tmpDir.resolve("output.mbtiles");
    BikeRouteOverlay.run(
        Arguments.of(
