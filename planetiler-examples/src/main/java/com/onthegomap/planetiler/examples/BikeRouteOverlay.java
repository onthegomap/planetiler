...
/**
 * Builds a map of bike routes from ways contained in OpenStreetMap relations tagged with <a
 * href="https://wiki.openstreetmap.org/wiki/Tag:route=bicycle">route=bicycle</a>.
 *
 ...
 *   <li>then run this example: {@code java -cp target/*-with-deps.jar
 *       com.onthegomap.planetiler.examples.BikeRouteOverlay osm_path="path/to/data.osm.pbf"
 *       mbtiles="data/output.mbtiles"}
 *   <li>then run the demo tileserver: {@code tileserver-gl-light --mbtiles data/bikeroutes.mbtiles}
...
public class BikeRouteOverlay implements Profile {
  ...
  // Minimal container for data we extract from OSM bicycle route relations. This is held in RAM so
  // keep it small.
  ...
  public void processFeature(SourceFeature sourceFeature, FeatureCollector features) {
    ...
    if (sourceFeature.canBeLine()) {
      ...
      for (var routeInfo : sourceFeature.relationInfo(RouteRelationInfo.class)) {
        ...
        // Break the output into layers named:
        // "{bicycle,route}-route-{international,national,regional,local,other}"
        ...
        features
            ...
            .setZoomRange(0, 14)
            // don't filter out short line segments even at low zooms because the next step needs
            // them
  ...
  public List<VectorTile.Feature> postProcessLayerFeatures(
    ...
    // FeatureMerge has several utilities for merging geometries in a layer that share the same
    // tags.
