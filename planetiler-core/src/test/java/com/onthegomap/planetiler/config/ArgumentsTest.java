...
public class ArgumentsTest {
  ...
  public void testMapBased() {
    assertEquals("value", Arguments.of("key", "value").getString("key", "key", "fallback"));
  }

  @Test
  public void testOrElse() {
    Arguments args =
        Arguments.of("key1", "value1a", "key2", "value2a")
  ...
  public void testGetConfigFileFromArgs() {
    Arguments args =
        Arguments.fromArgsOrConfigFile(
            "config=" + TestUtils.pathToResource("test.properties"), "key2=value2fromargs");
  ...
  public void testDefaultsMissingConfigFile() {
    Arguments args = Arguments.fromArgsOrConfigFile("key=value");
  ...
  public void testDuration() {
    Arguments args = Arguments.of("duration", "1h30m");
  ...
  public void testInteger() {
    Arguments args = Arguments.of("integer", "30");
  ...
  public void testLong() {
    ...
    Arguments args = Arguments.of("long", Long.toString(maxInt * 2));
  ...
  public void testList() {
    ...
    assertEquals(List.of("1"), Arguments.of().getList("list", "list", List.of("1")));
  }
  ...
  public void testFile() {
    assertNotNull(
        Arguments.of("file", TestUtils.pathToResource("test.properties"))
            .inputFile("file", "file", Path.of("")));
    assertThrows(
        IllegalArgumentException.class,
        () ->
            Arguments.of("file", TestUtils.pathToResource("test.Xproperties"))
                .inputFile("file", "file", Path.of("")));
    assertNotNull(
        Arguments.of("file", TestUtils.pathToResource("test.Xproperties"))
  ...
  public void testBounds() {
    assertEquals(
        new Envelope(1, 3, 2, 4),
        new Bounds(Arguments.of("bounds", "1,2,3,4").bounds("bounds", "bounds")).latLon());
    assertEquals(
        new Envelope(-180.0, 180.0, -85.0511287798066, 85.0511287798066),
        new Bounds(Arguments.of("bounds", "world").bounds("bounds", "bounds")).latLon());
    assertEquals(
        new Envelope(7.409205, 7.448637, 43.72335, 43.75169),
        new Bounds(Arguments.of().bounds("bounds", "bounds"))
            .setFallbackProvider(
                new OsmInputFile(TestUtils.pathToResource("monaco-latest.osm.pbf")))
            .latLon());
  }
  ...
  public void testArgsKeyPresentImplies() {
    Arguments args = Arguments.fromArgs("--force");
  ...
  public void testUnderscoreDashSame() {
    assertTrue(
        Arguments.fromArgs("--force-down-load=true").getBoolean("force_down_load", "force", false));
    assertTrue(
        Arguments.fromArgs("--force-download=true").getBoolean("force_download", "force", false));
    assertTrue(
        Arguments.fromArgs("--force_download=true").getBoolean("force-download", "force", false));
    assertTrue(
        Arguments.fromArgs("--force_download=true").getBoolean("force_download", "force", false));
  }
}
