...
public class ProgressLoggersTest {
  ...
  public void testLogWorkerPipeline() throws InterruptedException {
    ...
    var pipeline =
        WorkerPipeline.start("pipeline", Stats.inMemory())
            .fromGenerator(
                "reader",
                next -> {
    ...
    var loggers = ProgressLoggers.createForLocale(Locale.US).newLine().addPipelineStats(pipeline);
  ...
  private void assertEventuallyEquals(String expected, Supplier<String> actual)
