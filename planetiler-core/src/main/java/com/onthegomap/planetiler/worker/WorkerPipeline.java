...
/**
 * A mini-framework for chaining sequential steps that run in dedicated threads with a queue between
 * each.
 *
 * <p>For example:
 *
 ...
 * }</pre>
 *
 * <p>NOTE: to do any forking/joining, you must construct and wire-up queues and each sequence of
 * steps manually.
 *
...
public record WorkerPipeline<T>(
  ...
  /**
   * Returns a new pipeline builder where all worker and queue names will start with {@code
   * prefix_}.
   */
  ...
  /**
   * Blocks until all work has been completed by all steps of this pipeline, logging progress at a
   * fixed {@code logInterval}.
   *
  ...
  /**
   * Work that happens in a thread at the start of a pipeline to provide the initial elements to
   * process.
   *
  ...
  public interface SourceStep<O> {
     ...
     * @throws Exception if an error occurs, will be rethrown by {@link #await()} as a {@link
     *     RuntimeException}
     */
  ...
  /**
   * Work that happens in a thread in the middle of a pipeline that accepts elements from previous
   * steps and emits elements to the next step.
   *
  ...
  public interface WorkerStep<I, O> {
     ...
     *
     * @param prev get elements from the previous step using {@code prev.get}, will return null when
  ...
  /**
   * Work that happens in a thread at the end of a pipeline that accepts elements from the previous
   * step.
   *
   * @param <I> type of items that this step consumes
   */
  ...
  /**
   * An intermediate step while building a pipeline that requires the user to add a queue to buffer
   * items before adding the next step.
   *
  ...
  public interface Bufferable<E> {
    ...
    /**
     * Adds a {@link WorkQueue} that groups items into batches before enqueueing them to reduce
     * contention when many threads are reading or writing to the queue simultaneously.
     */
  ...
  public record Empty(String prefix, Stats stats) {
    ...
    /**
     * Adds an initial step that runs {@code producer} in {@code threads} worker threads to produce
     * items for this queue.
     */
    ...
    /**
     * Adds an initial step that runs {@code producer} in 1 worker thread to produce items for this
     * queue.
     */
    ...
    /**
     * Adds an initial step that reads all items from {@code iterable} in a single worker thread to
     * produce items for this queue.
     */
    ...
    /**
     * Populates an initial queue with {@code items} for subsequent steps to process but does not
    ...
    /**
     * Starts the pipeline from a queue that is populated externally, and does not kick off any
     * threads.
     */
  ...
  public record Builder<O>(
    ...
    private Builder(
        String prefix, String name, WorkQueue<O> outputQueue, Worker worker, Stats stats) {
    ...
    /**
     * Runs {@code step} simultaneously in {@code threads} threads which consumes items and emits
     * new ones that must be buffered.
     *
    ...
    /**
     * Runs {@code step} simultaneously in {@code threads} threads that consumes items but does not
     * emit any.
     */
    public WorkerPipeline<O> sinkTo(String name, int threads, SinkStep<O> step) {
      ...
      var worker =
          new Worker(
              prefix + "_" + name, stats, threads, () -> step.run(outputQueue.threadLocalReader()));
    ...
    /**
     * Runs {@code threads} simultaneous worker threads that consume items from previous step and
     * invoke {@code consumer.accept} for each one.
     */
