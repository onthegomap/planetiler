...
final class FilesArchiveUtils {
  ...
  static final String OPTION_WRITE_MODE = "write_mode";
  ...
  static WriteableFilesArchive.WriteMode writeMode(Arguments options) {
    final String mode = options.getString(
      OPTION_WRITE_MODE,
      "the write mode - either sync or async",
      WriteableFilesArchive.WriteMode.ASYNC.toString().toLowerCase()
    );
    return WriteableFilesArchive.WriteMode.valueOf(mode.toUpperCase());
  }
}
