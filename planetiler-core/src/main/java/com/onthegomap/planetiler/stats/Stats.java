...
/**
 * A utility that collects and reports more detailed statistics about the JVM and running tasks than
 * logs can convey.
 *
 * <p>{@link #inMemory()} stores basic stats in-memory to report at the end of the job and {@link
...
public interface Stats extends AutoCloseable {
  ...
  /**
   * Returns a new stat collector that stores basic stats in-memory to report through {@link
   * #printSummary()}.
   */
  ...
  /**
   * Returns a new stat collector pushes stats at a regular interval to a <a
   * href="https://github.com/prometheus/pushgateway">prometheus push gateway</a> at {@code
   * destination}.
   */
  ...
  /**
   * Logs top-level stats at the end of a job like the amount of user and CPU time that each task
   * has taken, and size of each monitored file.
   */
  ...
  /**
   * Records that a long-running task with {@code name} has started and returns a handle to call
   * when finished.
   *
   * <p>Also sets the "stage" prefix that shows up in the logs to {@code name}.
   */
  ...
  /**
   * Records that {@code numFeatures} features have been rendered to an output {@code layer} while
  ...
  /**
   * Records that a tile has been written to the mbtiles output where compressed size is {@code
   * bytes}.
   */
  ...
  /**
   * Returns and starts tracking a new counter with {@code name} optimized for the caller to
  ...
  /**
   * Returns and starts tracking a new counter where the value should be treated as number of
   * milliseconds.
   */
  ...
  /**
   * Tracks a group of counters with a {@code label} key that is set to the key in each entry of the
   * map returned from {@code values}.
   */
  void counter(String name, String label, Supplier<Map<String, Counter.Readable>> values);

  /**
   * Records that an invalid input feature was discarded where {@code errorCode} can be used to
   * identify the kind of failure.
   */
  void dataError(String errorCode);

  /**
   * A stat collector that stores top-level metrics in-memory to report through {@link
   * #printSummary()}.
   *
   * <p>Returns counters for the code to use, but doesn't keep a central listing of them.
   */
  class InMemory implements Stats {
    ...
    @Override
    public void counter(
        String name, String label, Supplier<Map<String, Counter.Readable>> values) {}
