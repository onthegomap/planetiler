...
/**
 * Utility that reads {@link SourceFeature SourceFeatures} from the geometries contained in an ESRI
 * shapefile.
 *
 * <p>Shapefile processing handled by geotools {@link ShapefileDataStore}.
 *
 * @see <a
 *     href="https://www.esri.com/content/dam/esrisites/sitecore-archive/Files/Pdfs/library/whitepapers/pdfs/shapefile.pdf">ESRI
 *     Shapefile Specification</a>
 */
public class ShapefileReader extends SimpleReader implements Closeable {

  private final FeatureCollection<SimpleFeatureType, org.opengis.feature.simple.SimpleFeature>
      inputSource;
  ...
  ShapefileReader(
      String sourceProjection, String sourceName, Path input, Profile profile, Stats stats) {
    super(profile, stats, sourceName);
    dataStore = open(input);
    try {
      String typeName = dataStore.getTypeNames()[0];
      FeatureSource<SimpleFeatureType, org.opengis.feature.simple.SimpleFeature> source =
  ...
  /**
   * Renders map features for all elements from an ESRI Shapefile based on the mapping logic defined
   * in {@code profile}. Overrides the coordinate reference system defined in the shapefile.
   *
   * @param sourceProjection code for the coordinate reference system of the input data, to be
   *     parsed by {@link CRS#decode(String)}
   * @param sourceName string ID for this reader to use in logs and stats
   * @param input path to the {@code .shp} file on disk, or a {@code .zip} file containing the
   ...
   */
  public static void processWithProjection(
  ...
  /**
   * Renders map features for all elements from an ESRI Shapefile based on the mapping logic defined
   * in {@code profile}. Infers the coordinate reference system from the shapefile.
   *
   * @param sourceName string ID for this reader to use in logs and stats
   * @param input path to the {@code .shp} file on disk, or a {@code .zip} file containing the
   *     shapefile components
    ...
    try {
      ...
      if (FileUtils.hasExtension(path, "zip")) {
        try (var zipFs = FileSystems.newFileSystem(path)) {
          Path shapeFileInZip =
              FileUtils.walkFileSystem(zipFs)
                  ...
                  .findFirst()
                  .orElseThrow(
                      () -> new IllegalArgumentException("No .shp file found inside " + path));
        ...
        throw new IllegalArgumentException(
            "Invalid shapefile input: " + path + " must be zip or shp");
      }
          ...
          if (latLonGeometry != null) {
            SimpleFeature geom =
                SimpleFeature.create(
                    latLonGeometry, new HashMap<>(attributeNames.length), sourceName, null, id);
